...
import { getCurrentPrice, getDefaultOptions, getLowestPrice, getRangeOfPrices } from "../utils/productUtils";
...
const Product = () => {
    ...
    useEffect(() => {
        (async () => {
            const found = testData[shopId]?.products?.find(({id}) => (id === parseInt(productId)))
            setProduct(found);
            if (found.variations) {
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                ...
                <div style={{display: 'flex', flexDirection: 'column', width: '500px', justifyContent: 'center', alignItems: 'center'}}>
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', gap: '5px', width: '200px'}}>
                        {product?.variations && product?.variations.map((variant) => (
                            <div style={{display: 'grid', gridTemplateColumns: 'auto auto'}}>
                                ...
                                <select onChange={({target: {value}}) => updateOption(variant.name, value)}>
                                    {variant.options.map((variantOption) => (
                                        <option value={variantOption.price}>{variantOption.name} =&gt; ${variantOption.price}</option>
                                    ))}
                                </select>
                            </div>
                        ))}
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', gap: '5px', width: '200px'}}>
                        ...
                        <button onClick={() => addToCart(shopId, product, selectedOptions)}>Add to Cart</button>
                    </div>
                </div>
            </div>
            <div>
